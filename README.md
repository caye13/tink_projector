...
Use another folder with:

```bash
python media_player.py --media-dir /path/to/videos
```

Test the projector-style fullscreen mode locally:

```bash
python media_player.py --media-dir /path/to/videos --fullscreen
```

Controls while playing:

- `Escape` or `q`: return to the library
- `Space`: pause/play
- Left/right arrows: seek ten seconds
- `F11`: toggle fullscreen from the library
...
The application does not write video frames through Python GPIO. On the Pi,
the TI configuration makes the Pi's 40-pin header a DPI/RGB666 display output.
The GTK window and GStreamer video are then rendered to that display normally.

After copying this project and TI's DLP230NP Python support package to the Pi:
...
The TI package also includes the EVM-specific `config.txt` timing configuration.
Apply that configuration before testing the video output. Do not run the TI
SPI flash-writing scripts while DPI video is active because the interfaces
share GPIO pins.
